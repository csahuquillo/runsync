...
> **Comparte un entreno en Strava, intervals.icu, Garmin Connect y Telegram con un solo tap desde la hoja de compartir del iPhone.**
...
Después de cada carrera repites el mismo trabajo aburrido en 4 sitios distintos:

- Renombrar la actividad en **Strava** y poner el modelo de zapatilla.
- Lo mismo en **intervals.icu**.
- Y otra vez en **Garmin Connect**.
- Exportar una imagen bonita desde **Bevel** y mandarla al grupo de Telegram con un caption tipo *"12 km — #Z2 #Base"*.
...
- Usas Android sin iOS → el Atajo es nativo de iOS.
...
Bevel exporta una imagen ─→ Hoja de compartir iOS ─→ Ejecutas el Atajo
                    ...
                    └──────────┬───────────┘
                               ▼
                    ┌──────────────────────┐
                    │ Menú: Zapatillas     │   "Adidas Boston 13", "NB More v5", …
                    └──────────┬───────────┘
                               ▼
                    ┌──────────────────────┐
...
└─────────────────┘   tags, Bearer    └──────────┬───────────────┘
              ...
              token         (PUT activity)   (garmin-     usuario)
              refresh                        connect lib)
```

- **iOS Shortcuts** (no se commitea en el repo, ver [docs/atajo-setup.md](docs/atajo-setup.md) para reconstruirlo).
- **Backend Python 3.12** con FastAPI/uvicorn, levantado por systemd.
- **TLS** via nginx + Let's Encrypt.
- **Sesiones persistentes** por servicio: OAuth de Strava se auto-refresca, Garmin guarda tokens, Telegram usa session de Telethon (no bot — mensajes como tu usuario).
...
| Construir el Atajo iOS siguiendo [la guía](docs/atajo-setup.md) | 30-45 min |
| **Total** | **~2-3 horas** |
...
1. **Clónate el repo y léete los docs.**
   ```bash
   git clone https://github.com/csahuquillo/runsync.git
   cd runsync
   ```
2. **Despliega el backend** siguiendo [docs/deploy.md](docs/deploy.md). Necesitas un VPS Linux con Python 3.12, nginx, y un dominio.
3. **Configura `/etc/runsync.env`** con tus credenciales — plantilla en [`runsync.env.example`](runsync.env.example).
4. **Bootstrap interactivo** de Strava (OAuth en navegador), Garmin (MFA email) y Telegram (código SMS). Ver [docs/deploy.md](docs/deploy.md#5-bootstrap-de-sesiones).
5. **Construye el Atajo en tu iPhone** siguiendo [docs/atajo-setup.md](docs/atajo-setup.md). ~40 min con la guía visual.
6. **Prueba** con `curl https://tudominio.com/health` desde otra máquina, y luego ejecuta el Atajo sobre una imagen de Bevel.
...

{
  ...
  "image_b64":      "iVBORw0KGgo...",        // imagen base64
...
```json
{
  "ok": true,
  "results": {
    "strava":    { "ok": true, "activity_id": 18606114945, ... },
    "intervals": { "ok": true, "activity_id": "i150530805", ... },
    "garmin":    { "ok": true, "activity_id": 22969608335, ... },
...
├── docs/
│   ├── architecture.md     ← arquitectura detallada + flujo de datos
│   ├── deploy.md           ← cómo desplegar desde cero
│   ├── atajo-setup.md      ← cómo construir el Atajo iOS paso a paso
│   └── atajo-pasos.md      ← guía de los arreglos clásicos cuando algo va mal
├── server/
...
El backend está organizado para que cada conector sea independiente. Borra el que no quieras en `connectors.py` y en `main.py` (líneas que llaman `<plataforma>_sync`). PRs bienvenidos para añadir más plataformas (Coros, Polar Flow, Suunto…).

### ¿Por qué JSON con base64 y no multipart?
iOS Shortcuts envía multipart de forma inconsistente. La imagen llegaba como texto y rompía el parser. JSON con base64 es 33% más pesado pero funciona el 100% de las veces. Ver [docs/architecture.md](docs/architecture.md).
...
- Bearer token único compartido entre Atajo y servidor.
- `/etc/runsync.env` con `mode 600`.
- Sin SSH al servidor en mi setup (AWS SSM) pero cualquier acceso normal funciona.
- Sin rate limiting hoy (deuda técnica, ver ROADMAP).
...
- **Idempotencia**: si ejecutas el Atajo dos veces para el mismo entreno, hoy renombra dos veces y envía dos veces a Telegram. Pendiente, ver ROADMAP.
